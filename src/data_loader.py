...
def filter_files_by_year(files, start_year, end_year):
    ...
    for file_name in files:
        # Extract year from filename (search for a four-digit year anywhere in the filename)
...
def main():
    ...
    start_year = st.number_input("Start Year", min_value=2020, max_value=2024, value=2022)
    end_year = st.number_input("End Year", min_value=2020, max_value=2024, value=2023)
    ...
    if uploaded_file:
        temp_file_path = DATA_DIR / f"temp_{uploaded_file.name}"
    ...
    if selected_files:
        ...
        for file_name in selected_files:
            file_path = DATA_DIR / (file_name if "temp_" not in file_name else f"temp_{file_name}")
